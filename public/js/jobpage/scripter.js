...
document.getElementById("radio-all-job-types").addEventListener         ("click",function(){FilterJobType(false, false, false, 'full-time', false)});
...
document.getElementById("radio-all-profession-levels").addEventListener ("click",function(){FilterProfessionLevel(false, false, false, false, false, false, 'internship-level', false)});
...
function FilterJobType(fullTimeState, partTimeState, internshipState, jobType, isFiltering) {
    ...
    if (isFiltering) {
        ...
        if (isFilteringInternshiplevel) {
            ...
            ShowJobBoxes(jobType, "internship-level", isFiltering, "");
            CheckIfNoBoxes();
        } 
        else if (isFilteringEntrylevel) {

            ShowJobBoxes(jobType, "entry-level-level", isFiltering, "");
            CheckIfNoBoxes();
        }
        else if (isFilteringAssociate) {

            ShowJobBoxes(jobType, "associate", isFiltering, "");
            CheckIfNoBoxes();
        }
        else if (isFilteringMidseniorlevel) {

            ShowJobBoxes(jobType, "mid-senior-level", isFiltering, "");
            CheckIfNoBoxes();
        }
        else if (isFilteringDirector) {

            ShowJobBoxes(jobType, "director", isFiltering, "");
            CheckIfNoBoxes();
        }
        else if (isFilteringExecutive) {

            ShowJobBoxes(jobType, "executive", isFiltering, "");
            CheckIfNoBoxes();
        }
        else 
        {
            ShowJobBoxes("", "", !isFiltering, jobType);
            CheckIfNoBoxes();
        }
    ...
    else if (!isFiltering) 
    {
        ...
        if (isFilteringInternshiplevel) {
            ...
            ShowJobBoxes(jobType, "", isFiltering, "internship-level");
            CheckIfNoBoxes();
        } 
        else if (isFilteringEntrylevel) {

            ShowJobBoxes(jobType, "", isFiltering, "entry-level-level");
            CheckIfNoBoxes();
        }
        else if (isFilteringAssociate) {

            ShowJobBoxes(jobType, "", isFiltering, "associate");
            CheckIfNoBoxes();
        }
        else if (isFilteringMidseniorlevel) {

            ShowJobBoxes(jobType, "", isFiltering, "mid-senior-level");
            CheckIfNoBoxes();
        }
        else if (isFilteringDirector) {

            ShowJobBoxes(jobType, "", isFiltering, "director");
            CheckIfNoBoxes();
        }
        else if (isFilteringExecutive) {

            ShowJobBoxes(jobType, "", isFiltering, "executive");
            CheckIfNoBoxes();
        }
        else 
        {
...
function FilterProfessionLevel(internshipLevelState, entryLevelState, associateState, midSeniorLevelState, directorState, executiveState, professionLevel, isFiltering) {
    ...
    if (isFiltering) {
        ...
        if (isFilteringFulltime) {
            ...
            ShowJobBoxes("full-time", professionLevel, isFiltering, "");
            CheckIfNoBoxes();
        }
        else if (isFilteringParttime) {

            ShowJobBoxes("part-time", professionLevel, isFiltering, "");
            CheckIfNoBoxes();
        }
        else if (isFilteringInternship) {

            ShowJobBoxes("internship", professionLevel, isFiltering, "");
            CheckIfNoBoxes();
        }
        else {

            ShowJobBoxes("", "", !isFiltering, professionLevel);
            CheckIfNoBoxes();
        }
    } 
    else if (!isFiltering) 
    {
        ...
        if (isFilteringFulltime) {
            ...
            ShowJobBoxes("", professionLevel, isFiltering, "full-time");
            CheckIfNoBoxes();
        }
        else if (isFilteringParttime) {

            ShowJobBoxes("", professionLevel, isFiltering, "part-time");
            CheckIfNoBoxes();
        }
        else if (isFilteringInternship) {

            ShowJobBoxes("", professionLevel, isFiltering, "internship");
            CheckIfNoBoxes();
        }
        else
